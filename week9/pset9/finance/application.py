...
def buy():
    ...
    if request.method == "POST":
        ...
        keyExist = lookup(request.form.get("symbol"))

        if amount.isnumeric() == False:
            return apology("must provide numeric value for shares", 400)
        else:
            if (float(amount) - int(float(amount))) <= 1.0 and (float(amount) - int(float(amount))) != 0.0:
                return apology("must provide an integer number of shares", 400)
            else:
                # ensure number of shares is valid
                if int(request.form.get("shares")) <= 0:
                    return apology("must provide valid number of shares (greater than 0)", 400)
                else:
                    amount = int(amount)
